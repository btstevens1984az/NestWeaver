...
## Simulated demo visualizations

> These GIFs are **simulated demo visualizations** generated by `scripts/generate_demo_gifs.py`. They are **not** real robot camera footage. Replace them with your own field recordings when you have hardware.

### 1. Navigating a living room (obstacle-aware path)

![Navigating living room — simulated](docs/media/demo-navigate.gif)

*Omnidirectional base tracing a SLAM-style path around furniture in a home interior (simulated).*

### 2. Picking up a water bottle

![Picking water bottle — simulated](docs/media/demo-pick-bottle.gif)

*Depth ROI + 6-DoF arm grasp sequence on a coffee-table bottle (simulated).*

### 3. Delivering the bottle

![Delivering bottle — simulated](docs/media/demo-deliver.gif)

*Fetch-and-carry handover to a person / table (simulated).*

### 4. Voice query while moving (local LLM / Ollama)

![Voice LLM while moving — simulated](docs/media/demo-voice-llm.gif)

*On-device Ollama answering a spoken question while the base continues to navigate (simulated).*

### 5. Tidying / putting an item away

![Tidying item — simulated](docs/media/demo-tidy.gif)

*Clutter item picked from the floor and returned to a shelf home (simulated).*
